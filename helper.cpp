...
namespace helper
{
    std::vector<std::string> formatColoredChunkBlock(const std::string& raw)
    {
        ...
        {
            lines.push_back("\033[33m" + jConfigCurrent["environment"].dump() + "\033[0m"); // Yellow
        }
        if (jConfigCurrent.contains("error"))
        {
            lines.push_back("\033[31m" + jConfigCurrent["error"].get<std::string>() + "\033[0m"); // Red
        }
        if (jConfigCurrent.contains("warning"))
        {
            lines.push_back("\033[33m" + jConfigCurrent["warning"].get<std::string>() + "\033[0m"); // Yellow
        }
        if (jConfigCurrent.contains("info"))
        {
            lines.push_back("\033[34m" + jConfigCurrent["info"].get<std::string>() + "\033[0m"); // Blue
        }

        if (lines.empty())
        {
            lines.push_back("\033[31m" + raw + "\033[0m"); // Red
        }
